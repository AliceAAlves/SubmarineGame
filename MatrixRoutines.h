...
public:
	...
	static void setRotationX(T spinInDegrees, T m[16])
	{
		...
		m[5] = cosf(spinInDegrees*ToRadians); 
		m[6] = -sinf(spinInDegrees*ToRadians);
		m[9] = sinf(spinInDegrees*ToRadians);
		m[10]= cosf(spinInDegrees*ToRadians);
	}
	static void setRotationY(T spinInDegrees, T m[16])
	{
		setToIdentity(m);

		m[0] = cosf(spinInDegrees*ToRadians); 
		m[2] = sinf(spinInDegrees*ToRadians);
		m[8] = -sinf(spinInDegrees*ToRadians);
		m[10]= cosf(spinInDegrees*ToRadians);
	}
	static void setRotationZ(T spinInDegrees, T m[16])
	{
		...
		m[0] = cosf(spinInDegrees*ToRadians); 
		m[1] = -sinf(spinInDegrees*ToRadians);
		m[4] = sinf(spinInDegrees*ToRadians);
		m[5]= cosf(spinInDegrees*ToRadians);
	}
