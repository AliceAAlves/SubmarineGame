...
glm::vec3* colFaceNormal = new glm::vec3(0, 0, 0);
int* numTris = new int;
...
void display()
{
	...
	if(!ScreenElements::paused) submarine->update(deltaTime);
	...
	glUniform1i(glGetUniformLocation(myShader->handle(), "activeCam"), cam.getActiveCam());
	...
	Torpedo* enemyTorpedo = nullptr;
	if (!ScreenElements::paused) enemyTorpedo = enemy->update(deltaTime, submarine);
	...
	for (int i = 0; i < enemyTorpedos.size(); i++) {
		if (!ScreenElements::paused) enemyTorpedos[i]->update(deltaTime);
		enemyTorpedos[i]->display(viewingMatrix, myShader);
		if (enemyTorpedos[i]->hasExploded()) to_delete_index = i;
	}
	if (to_delete_index >= 0) {
		Torpedo* pointer = enemyTorpedos[to_delete_index];
		delete pointer;
		enemyTorpedos.erase(enemyTorpedos.begin() + to_delete_index); 
	}


	to_delete_index = -1;
	for (int i = 0; i < torpedos.size(); i++) {
		if (!ScreenElements::paused) torpedos[i]->update(deltaTime);
		torpedos[i]->display(viewingMatrix, myShader);
		if (torpedos[i]->hasExploded()) to_delete_index = i;
	}
	if (to_delete_index >= 0) {
		Torpedo* pointer = torpedos[to_delete_index];
		delete pointer;
		torpedos.erase(torpedos.begin() + to_delete_index);
	}
	//cout << "torpedos size " << torpedos.size()<< endl;
	...
	for (int i = 0; i < explosions.size(); i++) {
		if (!ScreenElements::paused) explosions[i]->update(deltaTime, cam.getCamPos(submarine->getPos()));
		explosions[i]->display(viewingMatrix, frameAnimationShader);
		if (explosions[i]->isFinished()) to_delete_index = i;
	}
	if (to_delete_index >= 0) { 
		Explosion* pointer = explosions[to_delete_index];
		delete pointer;
		explosions.erase(explosions.begin() + to_delete_index); 
	}
	...
	if (!ScreenElements::paused || !ScreenElements::started || ScreenElements::gameFinished) amount += temp * deltaTime;
	if (amount > 6.23)
		amount = 0;
...
void init()
{
	...
	if (objLoader.loadModel("TestModels/box.obj", modelbox))//returns true if the model is loaded, puts the model in the model parameter
	{
		...
		modelbox.calcVertNormalsUsingOctree(0);  //the method will construct the octree if it hasn't already been created.
...
void processKeys()
{
	if (keys['W'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
	...
	if (!keys['W']) {
		submarine->setMovingForward(false);
	}
	if (keys['S'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
	...
	if (keys['A'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		submarine->turnLeft(deltaTime);
		cam.rotateCamLeft(submarine->getRot(), submarine->getRotSpeed(), deltaTime);
		keys['D'] = false;
	}
	if (keys['D'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		submarine->turnRight(deltaTime);
		cam.rotateCamRight(submarine->getRot(), submarine->getRotSpeed(), deltaTime);
		keys['A'] = false;
	}
	if (keys[VK_UP] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		submarine->moveUp(deltaTime);
		keys[VK_DOWN] = false;
	}
	if (keys[VK_DOWN] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		submarine->moveDown(deltaTime);
		keys[VK_UP] = false;
	}
	if (!keys[VK_DOWN] && !keys[VK_UP]) {
		submarine->setMovingVertically(false);
	}
	if (keys[VK_SPACE] && !ScreenElements::paused && !ScreenElements::gameOver) {
		if (submarine->readyToFire())
			torpedos.push_back(submarine->fireTorpedo());
		keys[VK_SPACE] = false;
	}
	if (keys['1'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		cam.setActiveCam(1, submarine->getRot());
	}
	if (keys['2'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		cam.setActiveCam(2, submarine->getRot());
	}
	if (keys['3'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		cam.setActiveCam(3, submarine->getRot());
	}
	if (keys['4'] && !ScreenElements::paused && !ScreenElements::gameOver)
	{
		cam.setActiveCam(4, submarine->getRot());
	}
	if (keys[VK_RETURN]) {
		enterKeyPressed = true;
	}
	else if (enterKeyPressed) {
		if (!ScreenElements::started) {
			ScreenElements::started = true;
			ScreenElements::paused = false;
		}
		else if (ScreenElements::gameOver || ScreenElements::gameFinished) {
			// reset both submarines
			submarine->reset();
			submarine->setPos(0, -5.5, -342);
			//submarine->updateTransform(0, 180, 0);
			submarine->setRotation(180);
			cam.setCamRot(submarine->getRot());
			enemy->reset();
			enemy->setPos(-60, -71, 207);

			// reset collectables
			for (int i = 0; i < collectables.size(); i++) {
				collectables[i]->resetThisCollectable();
				if (i < COLLECTABLES_ON_SCREEN) allActors.push_back(collectables[i]);
			}
			totalCollected = 0;
			ScreenElements::reset();
		}
		else if (ScreenElements::paused) {
			ScreenElements::paused = false;
		}
		else ScreenElements::paused = true;
...
void update()
{
	if (!ScreenElements::paused) {
		submarine->checkAllCollisions(allActors, colPoint, colPoint2, colFaceNormal, deltaTime, numTris);
		enemy->checkAllCollisions(allActors, colPoint, colPoint2, colFaceNormal, deltaTime, numTris);
		for (int i = 0; i < torpedos.size(); i++) {
	...
	if (totalCollected >= Collectable::totalCollectables) {
		ScreenElements::gameFinished = true;
		ScreenElements::paused = true;
	}
	if (ScreenElements::healthBar <= 0) {
		ScreenElements::gameOver = true;
	}
	//cout << screenElements.healthBar << endl;
}
