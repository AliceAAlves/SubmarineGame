...
void Font::make_dlist( FT_Face face, char ch, float color[3]) {
	...
	vert[0] =0; vert[1] =static_cast<float>(bitmap.rows); vert[2] =0.0; //x,y,z values for each vertex
	vert[3] =0; vert[4] =0; vert[5] =0.0;
	vert[6] = static_cast<float>(bitmap.width); vert[7] = static_cast<float>(bitmap.rows); vert[8]= 0.0;
	
	vert[ 9] = static_cast<float>(bitmap.width); vert[10] = static_cast<float>(bitmap.rows); vert[11]= 0.0;
	vert[12] = static_cast<float>(bitmap.width); vert[13] =0.0; vert[14]= 0.0;
	vert[15] = 0.0; vert[16] =0.0; vert[17]= 0.0;

	vert[0] += static_cast<float>(bitmap_glyph->left);
	vert[3] += static_cast<float>(bitmap_glyph->left);
	vert[6] += static_cast<float>(bitmap_glyph->left);
	vert[9] += static_cast<float>(bitmap_glyph->left);
	vert[12] += static_cast<float>(bitmap_glyph->left);
	vert[15] += static_cast<float>(bitmap_glyph->left);

	vert[1] += static_cast<float>((bitmap_glyph->top-bitmap.rows));
	vert[4] += static_cast<float>((bitmap_glyph->top-bitmap.rows));
	vert[7] += static_cast<float>((bitmap_glyph->top-bitmap.rows));
	vert[10]+= static_cast<float>((bitmap_glyph->top-bitmap.rows));
	vert[13]+= static_cast<float>((bitmap_glyph->top-bitmap.rows));
	vert[16]+= static_cast<float>((bitmap_glyph->top-bitmap.rows));
	...
	/********************************************************/
	float pos = static_cast<float>(face->glyph->advance.x >> 6);
	widthChar[ch] = pos;
}
...
void print(const Font &ft_font, float x, float y, const char *fmt, ...)  {
	...
	for(int i=0;i<static_cast<int>(lines.size());i++) {
		...
		for(int k=0; k<static_cast<int>(lines[i].length()); k++)
		{
